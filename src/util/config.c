...
bool check(char *line, char *arg){
	...
	// Fix later
	if(line == NULL){LOG(PARSE, "Line argument was NULL!");return false;}
	if(arg == NULL){LOG(PARSE, "Arg argument was NULL");return false;}
...
bool config(void *ptr, char *path, ConfigLoader func){
	...
	if(!func){LOG(IS_NULL, "Passed NULL loader to config"); return false;}

	FILE *f = fopen(path, "r");
	if(!f){LOG(ABORT, "Failed to open path %s. Aborting", path);}
...
char *format_path(char *base, char *format, int num){
	if(!base){LOG(IS_NULL, "Passed NULL base string");}
	if(!format){LOG(IS_NULL, "Passed NULL format string");}
	...
	if(!t_snprintf(NULL, 0, &needed, fmt, base, (unsigned int)num, format)){
		LOG(PARSE, "failed to compute length");
		return NULL;
	}
	...
	if(!t_snprintf(file, needed + 1, NULL, fmt, base, (unsigned int)num, format)){
		free(file);
		LOG(IS_NULL, "Failed to write buffer");
		return NULL;
	}
...
bool dir_exists(const char *path) {
    ...
    if (slash == NULL) {
        // no directory component — implies current directory
        return 1;
    }
    *slash = '\0'; // truncate at the last slash
